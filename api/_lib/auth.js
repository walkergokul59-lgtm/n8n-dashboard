...
export function userView(user, effectiveTier = 'free') {
  return {
    id: user.id,
    email: user.email,
    role: user.role,
...
function issueUserSession(user, effectiveTier = 'free') {
  ...
  return { token, user: userView(user, effectiveTier) };
}
...
export async function loginWithPassword(email, password) {
  ...
  return issueUserSession(user, effectiveTier);
}
...
export async function signupClientUser({ email, password, clientName }) {
  ...
  return issueUserSession(createdUser, 'free');
}
...
export async function requireUser(req) {
  ...
  const base = getAllowedWorkflowIds(config, user);
  const tierCapped = applyTierWorkflowLimit(base, effectiveTier);

  return { user, config, effectiveTier, access: { allowedWorkflowIds: tierCapped } };
}
