...
const ChromaGrid = ({ items, renderItem, className = '', radius = 300, damping = 0.45, fadeOut = 0.6, ease = 'power3.out' }) => {
    ...
    const baseClassName = className.includes('grid') ? className : `flex flex-wrap justify-center items-start gap-3 ${className}`;
    ...
    return (
        <div
            ...
            className={`relative w-full h-full ${baseClassName}`}
            style={{
                '--r': `${radius}px`,
                '--x': '50%',
                '--y': '50%'
        ...
        >
            {data.map((c, i) => {
                if (renderItem) {
                    return renderItem(c, i, handleCardMove, handleCardClick);
                }
                return (
                    <article
                        key={i}
